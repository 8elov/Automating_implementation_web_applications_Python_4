...
def createpost(token):
    ...
    p = requests.post('https://test-stand.gb.ru/gateway/posts',
                      headers={'X-Auth-Token': token},
                      data={'title': 'Новый пост для тестирования',
                            'description': 'informaition about post',
                            'content': 'Добавить еще один тест в задание для'
                                       'REST API, где необходимо выполнить'
                                       'следующие действия: создать новый пост'
                                       'и затем проверить его наличие на'
                                       'сервере, используя поле "описание".'})
    if p:
        return p.json()
    else:
...
def findpost(token):
    ...
    d = requests.get('https://test-stand.gb.ru/api/posts',
                     headers={'X-Auth-Token': token})
    if d:
